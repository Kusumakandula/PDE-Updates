...
def is_valid_username(username):
    return username.endswith("@randomtrees.com")
...
def login():
    ...
    if login_btn:
        if not is_valid_username(username):
            st.error("Invalid username. Must end with @randomtrees.com")
        elif not is_valid_password(password):
            st.error("Password must contain 1 capital, 1 small, 1 digit, 1 special char, and be 8–15 chars long")
        else:
...
def generate_styled_table_html(df):
    ...
    for _, row in df.iterrows():
        ...
        for col in columns:
            value = row[col]
            cell_content = "" if value is None or str(value).strip() == "" else str(value).strip()
...
def main():
    ...
    # ✅ LOGO ROW ONLY (Logout button removed from here)
    col1, col_spacer, col2, col3 = st.columns([2, 4, 2, 1])
    with col1:
        st.image("randomtrees.png", width=300)
    with col2:
        st.markdown(
    ...
    # ✅ ADD FIXED LOGOUT BUTTON AT BOTTOM-LEFT
    st.markdown(
        """
        <style>
        .logout-button {
    ...
    # ✅ Streamlit triggers logout via a dummy form; so we simulate with query param
    if st.query_params.get("logout") == "1":
        st.session_state["authenticated"] = False
        st.rerun()

    # ✅ JavaScript to simulate logout button redirect
    st.markdown("""
    <script>
